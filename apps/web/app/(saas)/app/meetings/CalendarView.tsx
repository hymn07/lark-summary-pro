...
export function CalendarView({
	...
	useEffect(() => {
		const handler = (e: MouseEvent) => {
			if (
				...
				!(e.target as HTMLElement).closest(".art-cell")
			) {
				setActiveDay(null);
				setIsExpanded(false);
			}
	...
	const changeMonth = (dir: number) => {
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				grid.className =
					"grid grid-cols-7 grid-rows-5 gap-y-1 grid-animator";
			}, 50);
		}, 250);
	};
	...
	const goBackToToday = () => {
		...
		setTimeout(() => {
			...
			setActiveDay(null);
			grid.className =
				"grid grid-cols-7 grid-rows-5 gap-y-1 grid-animator slide-left-in";
			setTimeout(() => {
				grid.className =
					"grid grid-cols-7 grid-rows-5 gap-y-1 grid-animator";
				const todayCell = document.getElementById(
					`art-trigger-${todayDate}`,
				);
				if (todayCell) {
					todayCell.classList.add("ping-active");
					setTimeout(
						() => todayCell.classList.remove("ping-active"),
						1600,
					);
				}
			}, 50);
	...
	const activeMeetings = activeDay
		? (meetingsByDate.get(activeDay) ?? [])
		: [];
		...
		<div className="w-full bg-slate-100/60 p-3 rounded-[32px] border border-white/40 shadow-sm relative">
			<div className="bg-white/80 backdrop-blur-md rounded-[24px] p-6 overflow-hidden">
				...
				<div className="relative overflow-hidden min-h-[400px]">
					<div
						ref={gridRef}
						className="grid grid-cols-7 grid-rows-5 gap-y-1 grid-animator"
					>
						{Array.from({ length: firstDayOfWeek }).map((_, i) => (
							<div
								key={`empty-${i}`}
								className="art-cell min-h-[90px]"
							/>
						))}
							...
							return (
								...
								>
									<div className="z-10 flex justify-between items-center w-full">
										<span
											className={`text-sm font-bold ${isToday ? "text-indigo-600" : "text-slate-700"}`}
										>
									...
									{count > 0 ? (
										<span
											className={`z-10 text-[10px] font-semibold tracking-wide px-1.5 py-0.5 rounded-md w-max ${
												isToday
													? "text-indigo-500 bg-indigo-50/60"
										...
										<span className="z-10 text-[10px] text-transparent">
											·
										</span>
									)}
								</div>
				...
				>
					...
					<div className="space-y-2 shrink-0">
						{visibleMeetings.map((m) => {
							...
							const startTime = m.startTime
								? new Date(m.startTime as string)
								: null;
							const endTime = m.endTime
								? new Date(m.endTime as string)
								: null;
							const records =
							...
							return (
								...
								>
									<div className="flex justify-between items-start">
										<h4 className="text-xs font-semibold text-slate-800 truncate max-w-[180px]">
											{topic}
										</h4>
									...
									<div className="flex items-center justify-between mt-2 text-[10px] text-slate-400">
										<span className="font-medium">
											{startTime
												? `${startTime.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}${endTime ? ` - ${endTime.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}` : ""}`
												: ""}
										</span>
										{recStatus === "completed" ? (
											...
											<span className="text-red-500 bg-red-50 px-1.5 py-0.5 rounded-md">
												失败
											</span>
										) : (
											<Button
					...
					{hasMore && (
						<>
							<div
								id="expanded-box"
								className={`more-meetings-container space-y-2 ${isExpanded ? "expanded" : ""}`}
							>
							...
							<div className="mt-4 pt-2.5 border-t border-slate-100 flex items-center justify-center shrink-0">
								...
								>
									<span>
										{isExpanded
											? "收起内容"
											: `展开更多 (还有 ${hiddenCount} 场)`}
									</span>
									<svg
										className={`w-2 h-2 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`}
										viewBox="0 0 24 24"
										fill="none"
										stroke="currentColor"
										strokeWidth="2"
