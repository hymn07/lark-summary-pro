...
export function MeetingDetailDialog({
	...
	const generateMutation = useMutation({
		mutationFn: (meetingId: string) =>
			orpcClient.meetings.generate({ feishuMeetingId: meetingId }),
		onSuccess: () => {
	...
	const handleFetchTranscript = async () => {
		...
		try {
			const result = (await orpcClient.meetings.fetchTranscript({
				id,
			})) as { transcriptFetched: boolean };
			toast[result.transcriptFetched ? "success" : "warning"](
				result.transcriptFetched
					? "逐字稿已获取，正在生成纪要"
					: "逐字稿尚未就绪，稍后重试",
			);
			queryClient.invalidateQueries({
	...
	const uploadMutation = useMutation({
		mutationFn: (text: string) =>
			orpcClient.meetings.uploadTranscript({ id: id!, text }),
		onSuccess: () => {
	...
	const deleteMutation = useMutation({
		mutationFn: (opts: { deleteRecords: boolean }) =>
			orpcClient.meetings.deleteFeishu({ id: id!, ...opts }),
		onSuccess: () => {
			toast.success("已删除");
			onOpenChange(false);
			queryClient.invalidateQueries({
						...
						<div className="flex-1 overflow-y-auto p-6 no-scrollbar bg-slate-50/40 space-y-6">
							...
							<div className="space-y-3.5">
								...
								{records.length === 0 ? (
									...
									<>
										<div
											className="space-y-3"
											id="main-summary-list"
										>
											{visibleRecords.map((r) => {
												const st = r.status as string;
												const Icon =
													STATUS_ICONS[
														st as keyof typeof STATUS_ICONS
													] ?? Loader2;
												const isProcessing =
													st === "processing";
												const isBad =
												...
												return (
													...
													>
														<div className="flex justify-between items-center mb-2.5">
															<span
																className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${isProcessing ? "text-blue-600 bg-blue-50" : isBad ? "text-red-500 bg-red-50" : "text-emerald-600 bg-emerald-50"}`}
															>
																<Icon
																	className={`h-3 w-3 mr-1 inline ${isProcessing ? "animate-spin" : ""}`}
																/>
																{isProcessing
																	? "处理中"
																	: isBad
											...
											<>
												...
												<div className="text-center pt-1">
													...
													>
														{isSummaryExpanded
															? "收起内容 ↑"
															: `加载更多 (还有 ${hiddenRecords.length} 份) ↓`}
													</button>
												</div>
											</>
										)}
							...
							<div className="space-y-3.5">
								...
								{transcriptText || userTranscriptText ? (
									...
									>
										<div
											className={`transcript-clamp-zone text-xs leading-relaxed text-slate-600 pr-1 no-scrollbar ${isTranscriptExpanded ? "expanded" : ""}`}
										>
				...
				<div className="modal-overlay-2 fixed inset-0 bg-slate-900/30 z-50 flex items-center justify-center p-4 active">
					<div className="modal-container-2 bg-white border border-slate-100 w-full max-w-sm rounded-[24px] shadow-[0_32px_80px_-16px_rgba(15,23,42,0.14)] overflow-hidden p-6">
						...
						<p className="text-xs text-slate-500 mb-4">
							该会议有 {records.length} 条关联纪要。是否同时删除？
						</p>
					...
					<div className="modal-container-2 bg-white border border-slate-100 w-full max-w-lg rounded-[32px] shadow-[0_32px_80px_-16px_rgba(15,23,42,0.14)] overflow-hidden flex flex-col">
						...
						<div className="p-6 space-y-4">
							<div className="space-y-1.5">
								...
								<div className="bg-slate-100 p-1 rounded-xl flex space-x-1 mb-2">
									<button
										type="button"
										className="tab-btn flex-1 py-1.5 text-[11px] font-medium rounded-lg text-indigo-600 bg-white shadow-sm transition-all active"
									>
									...
									<button
										type="button"
										className="tab-btn flex-1 py-1.5 text-[11px] font-medium rounded-lg text-slate-600 transition-all"
									>
										手动输入
									</button>
								</div>
						...
						<div className="px-6 py-4 border-t border-slate-100 flex justify-end space-x-2 bg-white">
							...
							<Button
								...
								onClick={() =>
									uploadMutation.mutate(uploadText.trim())
								}
							>
								{uploadMutation.isPending
									? "上传中..."
