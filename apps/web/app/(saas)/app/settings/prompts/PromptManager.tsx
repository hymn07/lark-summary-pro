...
export function PromptManager() {
  ...
  const handleCreate = () => {
    if (!newName.trim()) { toast.error("请填写版本名称"); return; }
    if (createMode === "ai") {
  ...
  if (isLoading) return (
    <div className="space-y-3 max-w-2xl">
      {[1,2,3].map(i => <Card key={i}><CardContent className="p-4"><Skeleton className="h-5 w-32 mb-2" /><Skeleton className="h-4 w-64" /></CardContent></Card>)}
    </div>
  );
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>版本名称</Label>
              <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="如：简洁版" />
            </div>
            {createMode === "ai" ? (
              <div>
                <Label>上传 1-3 篇示例纪要</Label>
                <p className="text-sm text-gray-500 mb-2">粘贴文本或上传文件</p>
                {samples.map((s, i) => (
              ...
              <>
                ...
                <div>
                  <Label>风格描述（可选）</Label>
                  <Input value={styleDesc} onChange={(e) => setStyleDesc(e.target.value)} placeholder="如：简洁分点式" />
                </div>
              </>
            )}
          </div>
      ...
      <div className="space-y-3">
        {versionList.length === 0 ? (
          <p className="text-gray-500 text-center py-8">还没有 Prompt 版本</p>
        ) : (
            ...
            <Card key={v.id as string}>
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => deleteMutation.mutate({ id: v.id as string })}><Trash2 className="h-4 w-4 text-red-500" /></Button>
                </div>
              </CardContent>
            </Card>
          ))
