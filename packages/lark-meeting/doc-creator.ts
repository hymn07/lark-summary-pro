...
import { getTenantAccessToken, addDocCollaborator } from "./feishu-client";
...
export async function createFeishuDoc(
  ...
  try {
    ...
    if (userId) {
      ...
      if (openId) {
        const added = await addDocCollaborator(documentId, openId);
        if (added) {
          console.log(`协作者已添加: ${documentId} → ${openId}`);
        }
      }
    }
